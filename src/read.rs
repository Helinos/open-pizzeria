...
pub fn read(path: &str) -> Result<(), FileParseError> {
        ...
        match chunk_mode {
            0 => {
                if chunk_id == 0x6666 {
                    ...
                    read_image_bank(image_bank_data)?;
                } else {
                    reader.skip(chunk_size);
                }
            }
            1 => {
                let _decompressed_size = reader.readu32();
...
fn read_image_bank(data: &[u8]) -> Result<(), FileParseError> {
    let mut image_bank_reader = Reader::new(data);
    ...
    for index in 0..number_of_items {
        ...
        let mut weird = false;
        if has_transparency {
            weird = size - color_size != color_size / 3;
        }
